...
The age-adjusted models estimate Female vs Male contrasts after adjusting for categorical age group only. Continuous outcomes are modeled with linear regression, and binary cutoff outcomes are modeled with logistic regression. The resulting forest plots show adjusted mean differences for continuous outcomes and adjusted odds ratios for binary outcomes.
...
![](../outputs/figures/fig1_bmi_by_sex.png){fig-cap="Figure 1. BMI distribution by sex. Crude boxplots compare the unadjusted distributions for Male and Female participants." fig-alt="Boxplot of BMI by sex."}
...
![](../outputs/figures/fig2_blood_glucose_by_sex.png){fig-cap="Figure 2. Blood glucose distribution by sex. Crude boxplots compare the unadjusted distributions for Male and Female participants." fig-alt="Boxplot of blood glucose by sex."}
...
![](../outputs/figures/fig3_age_group_percentages.png){fig-cap="Figure 3. Age-group percentages. This figure shows the composition of the analytic sample rather than a sex comparison." fig-alt="Bar chart of age-group percentages."}

The age distribution provides the context for the later adjusted models. If age groups are unevenly represented, crude sex comparisons can partly reflect sample composition rather than a direct difference in the measured outcomes.

![](../outputs/figures/fig4_cutoff_prevalence_by_sex.png){fig-cap="Figure 4. Prevalence above cutoff values by sex. Bars show crude Male and Female prevalences with approximate 95% confidence intervals." fig-alt="Horizontal grouped bar chart of cutoff prevalence by sex."}
...
![](../outputs/figures/fig5_age_adjusted_mean_differences.png){fig-cap="Figure 5. Age-adjusted mean differences for continuous outcomes. Estimates come from linear regression models adjusted for categorical age group only." fig-alt="Forest plot of age-adjusted mean differences."}

Figure 5 shows adjusted mean differences for the continuous outcomes. It answers a different question from the crude boxplots: not whether the raw distributions differ, but whether the Female vs Male contrast remains after allowing for age-group structure. Estimates close to zero indicate attenuation of the crude difference, while wider intervals indicate more uncertainty around the adjusted contrast.

![](../outputs/figures/fig6_age_adjusted_odds_ratios.png){fig-cap="Figure 6. Age-adjusted odds ratios for binary cutoff outcomes. Estimates come from logistic regression models adjusted for categorical age group only." fig-alt="Forest plot of age-adjusted odds ratios."}
...
![](../outputs/figures/fig7_interaction_continuous_outcomes.png){fig-cap="Figure 7. Sex-by-age patterns for continuous outcomes. This exploratory figure shows whether mean levels appear to vary differently by sex across age groups." fig-alt="Interaction line plots for continuous outcomes."}
